module Templatr
    ...
    scope :from_template, lambda {|template_id| where(:template_id => [nil, template_id]) }
	  scope :common_or_specific_type, lambda {|*type| where("template_id IS NULL OR template_id IN (?)", type.flatten) }
